  ...
  return (
    <div className="w-full flex flex-col min-h-screen">
      ...
      <div className="w-full flex flex-col gap-5 items-start p-[10px] px-5 md:px-10">
        <h1 className='text-2xl sm:text-3xl font-bold text-gray-500'>Best Shop in <span className='text-orange-600'>{city}</span></h1>
        <div className='w-full relative'>
          {showLeftShopBtn && (
            <button
              className="absolute left-0 top-1/2 -translate-y-1/2 bg-[#ff4d2d] text-white p-2 rounded-3xl shadow-lg hover:bg-[#e64528] z-10"
          ...
          <div className='w-full flex overflow-x-auto gap-4 pb-2' ref={shopScrollRef}>
            {shopsInCity?.map((shop, index) => (
              <CategoryCard image={shop.image.url} name={shop.restaurantName} key={index} onClick={() => navigate(`/shop/${shop._id}`)} />
            ))}
          </div>
          {showRightShopBtn && (
            <button
